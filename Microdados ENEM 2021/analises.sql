...
SELECT
	CASE WHEN in_treineiro = 0 THEN 'Não'
...
SELECT
	...
	COUNT(notas.nu_nota_cn) AS "quantidade",
	COUNT(notas.nu_inscricao)::numeric/(SELECT
							COUNT(nu_inscricao)
						FROM
							enem.presenca
						WHERE							
...
SELECT
	...
	COUNT(notas.nu_nota_ch) AS "quantidade",
	COUNT(notas.nu_inscricao)::numeric/(SELECT
							COUNT(nu_inscricao)
						FROM
							enem.presenca
...
SELECT
	...
	COUNT(notas.nu_nota_lc) AS "quantidade",
	COUNT(notas.nu_inscricao)::numeric/(SELECT
							COUNT(nu_inscricao)
						FROM
							enem.presenca
...
SELECT
	...
	END AS "Matemática",	
	COUNT(notas.nu_nota_mt) AS "quantidade",
	COUNT(notas.nu_inscricao)::numeric/(SELECT
							COUNT(nu_inscricao)
						FROM
							enem.presenca
...
SELECT
	...
	END AS "Redação",	
	COUNT(red.nu_nota_redacao) AS "quantidade",
	COUNT(red.nu_inscricao)::numeric/(SELECT
							COUNT(nu_inscricao)
						FROM
							enem.redacao
						WHERE							
...
WITH temp_notas AS (
	SELECT
		...
		nota.nu_nota_mt,
		red.nu_nota_redacao,
		(nota.nu_nota_cn + nota.nu_nota_ch + nota.nu_nota_lc + nota.nu_nota_mt + red.nu_nota_redacao)/5 AS "média simples"	
	FROM
		enem.presenca AS pre
		LEFT JOIN enem.notas AS nota
		ON pre.nu_inscricao = nota.nu_inscricao
		LEFT JOIN enem.redacao AS red
		ON pre.nu_inscricao = red.nu_inscricao
	WHERE 
		tp_presenca_cn = 1 AND tp_presenca_ch = 1 AND tp_presenca_lc = 1 AND tp_presenca_mt = 1
)
SELECT
	'Brazil' AS "País",
	esco.sg_uf_esc AS "Estado",
	COUNT(esco.nu_inscricao) AS quantidade,
	AVG(notas.nu_nota_cn)::int AS "Ciências da Natureza",
	AVG(notas.nu_nota_ch)::int AS "Ciências Humanas",
	AVG(notas.nu_nota_lc)::int AS "Linguagens e Códigos",
	AVG(notas.nu_nota_mt)::int AS "Matemática",
	AVG(notas.nu_nota_redacao)::int AS "Redação",
	AVG(notas."média simples")::int AS "Média simples"
FROM
	temp_notas AS notas
	LEFT JOIN enem.escolaridade AS esco
	ON notas.nu_inscricao = esco.nu_inscricao
WHERE
	esco.sg_uf_esc IS NOT NULL
GROUP BY
	"Estado"
ORDER BY
	"Estado"
